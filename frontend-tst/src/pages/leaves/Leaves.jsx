...
import { Space, Table, Tag, Button, Result,Input, Form, Select,DatePicker, Modal, message, Pagination } from "antd";
import "./leaves.css";
import {
  DeleteOutlined,
  EditOutlined,
...
const DataTable = () => {
  ...
  const handleSaveClick = (e) => {
    ...
    const url = "https://localhost:7046/api/Leave/EditLeave";
    axios
      .post(url, editData)
      .then((result) => {
        const data = result.data;
        if (data.statusCode === 200) {
          ...
          message.error("Leave Editing Failed");
        }
        setVisible(false);
      })
      .catch((error) => {
  ...
  <Form>
    ...
    <div className="form-group">
      ...
      <Select
              ...
              onChange={(value) => setEditData({ ...editData, leaveType: value})
            }
            > <Select.Option value={0} >Half Day Leave</Select.Option>
              <Select.Option value={1} >Annual Leave</Select.Option>             
              <Select.Option value={2} >Maternity Leave</Select.Option>
              <Select.Option value={3} >Sick Leave</Select.Option>
              <Select.Option value={4} >Unpaid Leave</Select.Option>
            </Select>
    </div>
    <br />
